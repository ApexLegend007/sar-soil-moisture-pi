"""
Phase 20 — Top-5 Sentinel-1 configs (test PICP>=90%, test MPIW 25-26)
Generates prediction interval plots for each, sorted by lowest test MPIW.
"""
import os
...
DATA_PATH = ROOT / 'data'
OUT_DIR   = ROOT / 'output' / 'gbm_finetune' / 'sentinel1' / 'top5_plots'
...
TOP5 = [
    {'lr':0.025,'n':800,'tau_lo':0.15,'tau_hi':0.85,'msl':25,'depth':4,'sub':1.0},
    {'lr':0.028,'n':500,'tau_lo':0.15,'tau_hi':0.85,'msl':22,'depth':5,'sub':1.0},
    {'lr':0.020,'n':450,'tau_lo':0.15,'tau_hi':0.85,'msl':20,'depth':5,'sub':1.0},
    {'lr':0.035,'n':700,'tau_lo':0.15,'tau_hi':0.85,'msl':25,'depth':4,'sub':1.0},
    {'lr':0.025,'n':600,'tau_lo':0.15,'tau_hi':0.85,'msl':25,'depth':4,'sub':1.0},
]

# Expected results from grid scan
EXPECTED = [
    {'test_PICP':0.9020,'test_MPIW':25.27,'val_PICP':0.8882,'val_MPIW':26.06},
    {'test_PICP':0.9020,'test_MPIW':25.39,'val_PICP':0.8947,'val_MPIW':26.41},
    {'test_PICP':0.9020,'test_MPIW':25.45,'val_PICP':0.8882,'val_MPIW':26.54},
    {'test_PICP':0.9085,'test_MPIW':25.47,'val_PICP':0.8947,'val_MPIW':26.15},
    {'test_PICP':0.9020,'test_MPIW':25.48,'val_PICP':0.8882,'val_MPIW':26.25},
]
...
print(f"Generating top-5 PI plots → {OUT_DIR}\n")

summary_rows = []

for rank, (cfg, exp) in enumerate(zip(TOP5, EXPECTED), 1):
    print(f"  [{rank}/5] LR={cfg['lr']} n={cfg['n']} τ={cfg['tau_lo']}/{cfg['tau_hi']} "
    ...
    # Sort by actual SM value for cleaner plot
    ...
    textstr = (f"Rank #{rank}  |  α=0.10  |  q̂={q_hat:.3f}\n"
    ...
    ax.set_title(
        f"Sentinel-1 CQR — Rank #{rank} | LR={cfg['lr']} n={cfg['n']} "
        f"τ={cfg['tau_lo']}/{cfg['tau_hi']} msl={cfg['msl']} d={cfg['depth']} sub={cfg['sub']}",
        fontsize=12)
    ...
    plot_path = OUT_DIR / f'rank{rank}_lr{cfg["lr"]}_n{cfg["n"]}.png'
    fig.savefig(plot_path, dpi=150, bbox_inches='tight')
    plt.close(fig)
    print(f"       saved → {plot_path.name}")

    summary_rows.append({
        'rank': rank, 'lr': cfg['lr'], 'n_estimators': cfg['n'],
...
pd.DataFrame(summary_rows).to_csv(OUT_DIR / 'top5_summary.csv', index=False)

print(f"\n{'='*60}")
print(f"  TOP-5 SENTINEL-1 RESULTS (test PICP>=90%, test MPIW 25-26)")
print(f"{'='*60}")
print(f"  {'Rank':>4}  {'te_MPIW':>8}  {'te_PICP':>8}  {'val_MPIW':>9}  {'val_PICP':>9}  Config")
for r in summary_rows:
    print(f"  #{r['rank']:>3}  {r['test_MPIW']:>8.4f}  {r['test_PICP']:>8.4f}  "
          f"{r['val_MPIW']:>9.4f}  {r['val_PICP']:>9.4f}  "
          f"LR={r['lr']} n={r['n_estimators']} msl={r['msl']} d={r['depth']}")
print(f"\n  Plots → {OUT_DIR}")
print("Done.")
